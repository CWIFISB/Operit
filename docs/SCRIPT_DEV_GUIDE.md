...
这是普通 Sandbox Package 作者的默认流程。先更新并运行 `SandboxPackage_DEV` 安装脚本，再在 `/storage/emulated/0/Android/media/com.ai.assistance.operit/dev_package/{packageId}/` 中开发。宿主类型放在同级的 `/storage/emulated/0/Android/media/com.ai.assistance.operit/dev_package/types/`，具体步骤以安装后的 `SKILL.md` 为准。
